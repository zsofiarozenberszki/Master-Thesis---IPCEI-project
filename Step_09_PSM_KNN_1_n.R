...
output_dir <- file.path(root_path, "z_Matching_result_files", "Nearest_Neighbour_matching_1_n")
...
specifications_2 <- list(
  Spec1 = c("Age", "std_Number of employees 2023", "std_Total assets th EUR 2018", "std_Operating revenue (Turnover) th EUR 2018", "std_ROA using Profit (Loss) before tax 2018"),
  Spec2 = c("Age", "std_Number of employees 2023", "std_Total assets th EUR 2018", "std_Operating revenue (Turnover) th EUR 2018"),
  Spec3 = c("Age", "std_Number of employees 2023", "std_ROA using Profit (Loss) before tax 2018", "std_Operating revenue (Turnover) th EUR 2018"),
...
for (spec in names(specifications_2)) {
  ...
  match_model <- matchit(formula_obj, data = psm_df, method = "nearest", ratio = 2)
  matched_data <- match.data(match_model)
...
# ========================
cat("\n================ OPTION 3 ================\n")
specifications_3 <- specifications_2  # same structure as OPTION 2
plots_3 <- list()
for (spec in names(specifications_3)) {
  ...
  psm_df <- combined_df %>%
    ...
    na.omit()
  formula_obj <- as.formula(paste("Treatment ~", paste0("`", vars, "`", collapse = " + ")))
  match_model <- matchit(formula_obj, data = psm_df, method = "nearest", ratio = 2, exact = ~ sector_2digit)
  matched_data <- match.data(match_model)
...
specifications_5 <- list(
  Spec1 = c("Age", "std_Number of employees 2023", "std_Total assets th EUR 2018", "std_Operating revenue (Turnover) th EUR 2018", "std_ROA using Profit (Loss) before tax 2018", "std_Research & development expenses th EUR 2018"),
  Spec2 = c("Age", "std_Number of employees 2023", "std_Total assets th EUR 2018", "std_Operating revenue (Turnover) th EUR 2018", "std_Research & development expenses th EUR 2018"),
  Spec3 = c("Age", "std_Number of employees 2023", "std_ROA using Profit (Loss) before tax 2018", "std_Operating revenue (Turnover) th EUR 2018", "std_Research & development expenses th EUR 2018"),
  Spec4 = c("Age", "std_Number of employees 2023", "std_Operating revenue (Turnover) th EUR 2018", "std_Research & development expenses th EUR 2018")
)
